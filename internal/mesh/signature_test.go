...
func TestSelfReject(t *testing.T) {
	...
	mk := func(notAfter int64) []byte {
		blob, err := signature.Sign(priv, sub, now.Add(-time.Hour).Unix(), notAfter, "", nil)
		must.NoError(t, err)
		return blob
	}
...
func TestReloadSignersFromFile(t *testing.T) {
	...
	blob, err := signature.Sign(priv, sub, time.Now().Add(-time.Hour).Unix(), time.Now().Add(time.Hour).Unix(), "", nil)
	must.NoError(t, err)
...
func TestApplySelfGrant(t *testing.T) {
	...
	old, err := signature.Sign(priv, sub, now.Add(-time.Hour).Unix(), now.Add(time.Minute).Unix(), "", nil)
	must.NoError(t, err)
	m.selfGrant.Store(&old)
	m.selfExpired.Store(true)

	renewed, err := signature.Sign(priv, sub, now.Add(-time.Minute).Unix(), now.Add(time.Hour).Unix(), "", nil)
	must.NoError(t, err)
	...
	expired, err := signature.Sign(priv, sub, now.Add(-time.Hour).Unix(), now.Add(-time.Minute).Unix(), "", nil)
	must.NoError(t, err)
	...
	rogue, err := signature.Sign(otherPriv, sub, now.Add(-time.Minute).Unix(), now.Add(time.Hour).Unix(), "", nil)
	must.NoError(t, err)
...
func TestApplySelfGrant_RejectsUnauthorizedSelfRoute(t *testing.T) {
	...
	routeless, err := signature.Sign(priv, sub, now.Add(-time.Minute).Unix(), now.Add(time.Hour).Unix(), "", nil)
	must.NoError(t, err)
	...
	scoped, err := signature.Sign(priv, sub, now.Add(-time.Minute).Unix(), now.Add(time.Hour).Unix(), "", nil, netip.MustParsePrefix("10.0.0.0/8"))
	must.NoError(t, err)
