...
func TestPolicyFilter(t *testing.T) {
	...
	must.NoError(t, err)
	kept, refused = policyFilter(peer, id, idOnly)
	...
	must.NoError(t, err)
	kept, refused = policyFilter(peer, id, keepID)
	...
	must.NoError(t, err)
	kept, refused = policyFilter(peer, id, block)
	...
	must.NoError(t, err)
	kept, refused = policyFilter(peer, id, dropID)
...
func TestPolicyFilter_AddressMatchesOnlyExactHostRoute(t *testing.T) {
	...
	kept, refused := policyFilter(peer, netip.MustParseAddr("fdcc::1"), idOnly)
