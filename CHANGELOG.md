...
* A node's tags are now operator-signed via `sign --tag k=v` in its grant, not the daemon `--tag` flag (removed). Verified at every peer like the signed name, so they can be trusted and are usable in `--peer-policy` and `--firewall-rules` (`peer.tags["role"]`); re-sign nodes with `--tag` to keep them tagged.
* A node's WireGuard endpoint is now the operator-signed `sign --endpoint <host:port>` carried in its grant, not the daemon `--endpoint` flag (removed), so a peer cannot redirect a node's tunnel; re-sign nodes with `--endpoint` to keep them reachable. The `--persistent-keepalive` flag is also removed.
...
* Firewall: a dedicated nftables table (on by default) scopes the gossip port to the wg device, and `--firewall-rules` adds local, `SIGHUP`-reloadable microsegmentation as an expr returning `allow(proto, ports, cond?)` rules that default-deny traffic to this node; `--disable-firewall` turns it off.
